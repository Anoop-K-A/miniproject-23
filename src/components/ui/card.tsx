...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card/95 text-card-foreground flex flex-col gap-6 rounded-2xl border border-border/70 shadow-[0_14px_34px_rgba(15,38,65,0.1)] backdrop-blur-sm",
        className,
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center gap-2 px-6 pb-6 [.border-t]:pt-5",
        className,
      )}
      {...props}
    />
